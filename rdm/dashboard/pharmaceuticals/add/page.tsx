...
import { usePharmaceuticalsContext } from '@/lib/providers/pharmaceuticals.context';
import { useLocalStorage } from '@/hooks/use-local-storage';

export default function AddPharmaceutical() {
  ...
  const { AddPharmaceuticalHandler } = usePharmaceuticalsContext();
    ...
    try {
      ...
      const userCredential = await createUserWithEmailAndPassword(
        ...
        .then(async (userCredential) => {
          ...
          try {
            // const res = await AddPharmaceuticalHandler({
            //   uid: newUser?.uid,
            //   firstName,
            //   lastName,
            //   email,
          ...
          router.push(`/${user?.uid}/pharmaceuticals`);
        })
        .catch((error) => {
  ...
  return (
    <div className="container mx-auto mt-8">
      <h1 className="mb-4 text-2xl font-semibold">Add Pharmaceutical</h1>
      ...
      <form onSubmit={handleSubmit} className="max-w-sm">
        ...
        >
          {loading ? <Loader /> : 'Add Pharmaceutical'}
        </button>
      </form>
    </div>
  );
